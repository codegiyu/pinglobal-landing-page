...
export async function POST(req: NextRequest) {
  try {
    ...
    const senderName =
      ...
      'Pin Global Website';
    const replyName = senderName === 'Pin Global Website' ? '' : senderName;

    const mailOptions = {
      from: `${senderName} (From Pin Global Website) <${process.env.fromEmail}>`,
      to: `Pin Global <${process.env.toEmail}>`,
